...
class ParetoScatter(QWidget):
    ...
    _PAD_L = 38   # left padding for y-axis label
    _PAD_B = 22   # bottom padding for x-axis label
    _PAD_T = 14   # top padding for title
    _PAD_R = 8

    def __init__(self, parent=None) -> None:
        super().__init__(parent)
        self._points: list[dict] = []
        self._px: list[float] = []   # pixel x coords for hit-testing
        self._py: list[float] = []
    ...
    def _to_pixel(self, w: int, h: int, x0: float, x1: float, y0: float, y1: float,
    ...
    def paintEvent(self, event) -> None:
        ...
        try:
            ...
            painter.setFont(sf)
            painter.setPen(_C_TEXT)
            painter.drawText(QRectF(pad_l, h - pad_b, (w - pad_l - pad_r) / 2, pad_b),
                             Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop,
                             f"{x0:.3g}")
            painter.drawText(QRectF(pad_l + (w - pad_l - pad_r) / 2, h - pad_b,
                                    (w - pad_l - pad_r) / 2, pad_b),
                             Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop,
                             f"{x1:.3g}")
            painter.drawText(QRectF(0, pad_t, pad_l - 2, 12),
                             Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop,
                             f"{y1:.3g}")
            painter.drawText(QRectF(0, h - pad_b - 12, pad_l - 2, 12),
                             Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop,
                             f"{y0:.3g}")
            # title
            tf = QFont(); tf.setPointSize(7)
            painter.setFont(tf)
            painter.drawText(pad_l + 2, pad_t - 2, f"Pareto ({len(self._points)})  "
                             f"[Obj1 vs Obj2, color=fitness]")
            # points colored by fitness (blue=low → green=high)
            self._px = []
            self._py = []
            painter.setPen(Qt.PenStyle.NoPen)
            for i, p in enumerate(self._points):
                px, py = self._to_pixel(w, h, x0, x1, y0, y1,
                                        p["objectives"][0], p["objectives"][1])
                self._px.append(px)
                self._py.append(py)
                rel = (p.get("fitness", f0) - f0) / fspan
                color = QColor.fromHsvF(0.33 * rel, 0.7, 0.9)
                painter.setBrush(QBrush(color))
...
class MapElitesHeatmap(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._cell_rects: list[tuple[float, float, float, float]] = []  # (x,y,cw,ch) per cell
        self.setMinimumHeight(110)
        self.setMouseTracking(True)
    ...
    def paintEvent(self, event) -> None:
        ...
        try:
            ...
            sf = QFont(); sf.setPointSize(7)
            painter.setFont(sf)
            painter.drawText(pad + 2, pad + title_h - 2,
                             f"MAP-Elites ({len(self._cells)})  "
                             f"[fitness: {min_fit:.3g} → {max_fit:.3g}]")
        finally:
            painter.end()
